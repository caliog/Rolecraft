...
public class NPC extends org.caliog.Rolecraft.XMechanics.npclib.NPC {
	...
	public void actAsHurt() {
		try {
			Object entity = getEntity();
			Object world = entityClass.getField("world").get(entity);
			worldServerClass.getMethod("broadcastEntityEffect", entityClass, byte.class).invoke(worldServerClass.cast(world), entity,
					(byte) 2);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void moveTo(Location l) {
		...
		Object entity = getEntity();
		try {
			entityClass.getMethod("setPositionRotation", double.class, double.class, double.class, float.class, float.class).invoke(entity,
	...
	public void updateEquipment() {
		try {
			...
			Constructor<?> packetConstr = packetPlayOutEntityEquipmentClass.getConstructor(int.class, enumItemSlotClass, itemstackClass);
			Object entity = getEntity();
			int changes = 0;
			...
			for (Object i : values) {
				...
				if (current == null) {
					if (previous != null) {
						Object packet = packetConstr.newInstance(entityClass.getMethod("getId").invoke(entity), i, current);
