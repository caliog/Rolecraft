...
public abstract class Spell {
	...
	public boolean execute() {
		if (this.player.getLevel() < getMinLevel()) {
			Msg.sendMessage(this.player.getPlayer(), Key.SKILL_NEED_LEVEL);
			return false;
		}
		if (this.player.getPlayer().getFoodLevel() - getFood() >= 0) {
			...
			Msg.sendMessage(this.player.getPlayer(), Key.SKILL_NEED_MANA);
			return false;
		}
		if (isActive()) {
			Msg.sendMessage(this.player.getPlayer(), Key.SKILL_ACTIVE);
			return false;
		}
		return true;
	}
